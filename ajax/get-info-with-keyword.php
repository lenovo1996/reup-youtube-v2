<?php
	if (isset($_POST['keyword'])) {
		$file = fopen('datas/logs/log_keyword.txt', 'a');
		fwrite($file, $_POST['keyword'] .':log_keyword:');
		fclose($file);
		$page = (isset($_POST['next_page'])) ? $_POST['next_page'] : '';
		$source = CURL('https://www.googleapis.com/youtube/v3/search?part=snippet&key=' . config('google_key_api') . '&q='.urlencode($_POST['keyword']).'&maxResults=20&type=video&pageToken=' . $page);
		...
		json(false, ['message' => 'Vui lòng nhập keyword']);
	}